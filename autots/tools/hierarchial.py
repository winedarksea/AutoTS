...
class hierarchial(object):
    ...
    Args:
        ...
        hier_ids (dict): dict of series_id: group_id to use if grouping is 'User'
    """
    ...
    def __init__(self, grouping_method: str = 'tile',
        ...
        self.hier_ids = hier_ids

        if self.grouping_method == 'user':
            if hier_ids is None:
                raise ValueError("hier_ids must be provided.")

    def fit(self, df):
        """Construct and save object info."""
        # construct hier_ids if not given
        hier_ids = self.hier_ids
        if self.grouping_method != 'user':
            num_hier = df.shape[1] / self.n_groups
            if self.grouping_method == 'tile':
                hier_ids = np.tile(np.arange(self.n_groups), int(np.ceil(num_hier)))[:df.shape[1]]
            elif self.grouping_method == 'alternating':
                hier_ids = np.repeat(np.arange(self.n_groups), int(np.ceil(num_hier)))[:df.shape[1]]
                ...
                hier_ids = kmeans.labels_
            elif self.grouping_method == 'dbscan':
                X = df.mean().values.reshape(-1, 1)
                from sklearn.cluster import DBSCAN
                clustering = DBSCAN(eps=1, min_samples=2).fit(X)
                hier_ids = clustering.labels_
            hier_ids = hier_ids.astype(str).astype(np.object)
            # z is a deliberate typo to make such an id very rare in source
            hier_ids = hier_ids + '_hierarchy_levelz'
            hier_ids = dict(zip(df.columns.tolist(), hier_ids))
            self.hier_ids = hier_ids

        self.top_ids = set(hier_ids.values())
        self.bottom_ids = hier_ids.keys()

        hier = df.abs().groupby(hier_ids, axis=1).sum()
        self.hier = hier

        if self.reconciliation == 'mean':
            level_sums = pd.DataFrame(hier.sum(axis=0))
            individal_sums = pd.DataFrame(df.abs().sum(axis=0))
            divisors = pd.DataFrame.from_dict(hier_ids, orient='index')
    ...
    def reconcile(self, df):
        ...
        if self.reconciliation is None:
            ...
            bottom_up = fore[self.bottom_ids].abs().groupby(self.hier_ids, axis=1).sum()
...
test = hierarchial(n_groups=3, grouping_method='dbscan',
                   hier_ids=hier_ids, reconciliation='mean').fit(df)
