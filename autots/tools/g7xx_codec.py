...
class G711Scaler(EmptyTransformer):
    ...
    def get_new_params(method: str = "random"):
        ...
        return {
            ...
            "mu": random.choices([50.0, 75.0, 100.0, 150.0, 200.0, 255.0], 
                                [0.25, 0.25, 0.2, 0.15, 0.1, 0.05])[0] if mode == "mu" else 100.0,
            "A": random.choice([50.0, 75.0, 87.6, 100.0]) if mode == "a" else 87.6,
            "center": random.choices(["median", "mean"], [0.8, 0.2])[0],
            "scale_method": random.choices(["mad", "std", "maxabs", "percentile"], [0.5, 0.2, 0.1, 0.2])[0],
            "scale_factor": random.uniform(1.5, 3.5),
            "min_scale": 1e-6,
            "clip": random.choice([True, True, False]),
            "zero_offset": random.choice([0.0, 0.0, 0.0, 1e-6, 1e-8]),
            "fill_method": random.choices(["interpolate", "ffill", "median"], [0.6, 0.3, 0.1])[0],
            "on_transform": True,
            "on_inverse": True,
            "bounds_only": False,
        }
