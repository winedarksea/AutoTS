...
class FeatureDetectionOptimizer:
    ...
    def _mutate_params(self, params, sampler, rng, allowed_keys=None):
        ...
        # These blocks are generated by detector-specific samplers and should be
        # replaced atomically to avoid method/method_params incompatibilities.
        atomic_sample_keys = {
            'rough_seasonality_params',
            'seasonality_params',
            'holiday_params',
        ...
        for selected_key in rng.sample(shared_keys, n_blocks):
            ...
            if selected_key == 'changepoint_params':
                ...
                if rng.random() < 0.6:
                    ...
                    mutated[selected_key] = copy.deepcopy(fresh_value)
            elif selected_key in atomic_sample_keys:
                mutated[selected_key] = copy.deepcopy(fresh_value)
    ...
    def fine_tune_changepoints(
        ...
        trend_segment_weight : float
            Low-weight symmetric penalty comparing detected and true trend
            component levels within true trend segments. This preserves the
            changepoint-first objective while discouraging solutions that place
            breaks correctly but collapse most trend mass into noise.
        ...
        for stage_idx, sigma in enumerate(curriculum_sigmas):
            ...
            stage_best, stage_loss, stage_entries = self._run_changepoint_stage(
                ...
                trend_segment_weight=trend_segment_weight,
            )
    ...
    def _evaluate_changepoint_params(
        ...
        """
        Score a parameter config using date proximity/count plus a small
        segment-level trend calibration term.
        ...
        for name in series_names:
            ...
            cp_loss += trend_segment_weight * self._trend_segment_level_penalty(
                detected_components.get(name, {}).get('trend'),
                true_components.get(name, {}).get('trend'),
                true_cp_entries,
                getattr(self.synthetic_generator, 'date_index', None),
            )
    ...
    @staticmethod
    def _trend_segment_level_penalty(
        detected_trend,
        true_trend,
        true_cp_entries,
        date_index=None,
        ...
        """
        Compare trend level inside true changepoint-defined segments.

        This is intentionally low-capacity and low-weight: it nudges the search
        away from decompositions that place changepoints correctly while pushing
        most low-frequency signal out of the trend component. It stays
        level-oriented on purpose rather than comparing segment variance or
        shape, which would start acting like a soft slope objective.
        """
        if detected_trend is None or true_trend is None:
            return 0.0

        detected_arr = np.asarray(detected_trend, dtype=float).reshape(-1)
        true_arr = np.asarray(true_trend, dtype=float).reshape(-1)
        ...
        if date_index is not None:
            try:
                index = pd.DatetimeIndex(date_index[:length])
            except Exception:
                index = None
        else:
            index = None
        ...
        segment_starts = [0]
        if true_cp_entries:
            for entry in true_cp_entries:
                cp_date = pd.Timestamp(entry[0])
                if index is not None:
                    loc = index.searchsorted(cp_date, side='left')
                    cp_idx = int(min(max(loc, 0), length - 1))
                else:
                    continue
                if 0 < cp_idx < length:
                    segment_starts.append(cp_idx)
        segment_starts = sorted(set(segment_starts))
        segment_bounds = list(zip(segment_starts, segment_starts[1:] + [length]))

        overall_scale = float(np.nanstd(true_arr))
        if not np.isfinite(overall_scale) or overall_scale < 1e-6:
            overall_scale = float(np.nanmean(np.abs(true_arr))) + 1e-6
        overall_scale = max(overall_scale, 1e-6)

        penalties = []
        for start, end in segment_bounds:
            if (end - start) < 3:
                continue
            det_seg = detected_arr[start:end]
            true_seg = true_arr[start:end]
            mask = np.isfinite(det_seg) & np.isfinite(true_seg)
            if mask.sum() < 3:
                continue
            det_seg = det_seg[mask]
            true_seg = true_seg[mask]

            true_mean = float(np.nanmean(true_seg))
            det_mean = float(np.nanmean(det_seg))
            mean_scale = max(abs(true_mean), overall_scale, 1e-6)
            mean_penalty = abs(det_mean - true_mean) / mean_scale
            penalties.append(min(mean_penalty, 3.0))
