...
class TestScaling(unittest.TestCase):
    ...
    def test_standardize_true_impacts_are_unscaled(self):
        ...
        if 'components' in detected and detected['components']:
            ...
            if 'seasonality' in components:
                ...
                if isinstance(seasonality, dict):
                    ...
                    if values:
                        ...
                        self.assertGreater(max_seasonal, original_std * 0.01,
            ...
            if 'holidays' in components:
                ...
                if isinstance(holidays, dict):
                    values = [v for v in holidays.values() if isinstance(v, (int, float))]
                    if values:
                        max_holiday = max(abs(v) for v in values)
                        self.assertGreater(max_holiday, original_std * 0.01,
        ...
        if detector.reconstructed is not None:
            ...
            self.assertAlmostEqual(recon_mean, original_mean, delta=original_std,
            ...
            self.assertAlmostEqual(recon_std, original_std, delta=original_std * 0.5,
                                 msg=f"Reconstructed std {recon_std} doesn't match original std {original_std}")
            
            # Values should be in same range as original
            self.assertGreater(reconstructed_series.min(), self.data_scaled[series_name].min() - original_std * 3)
            self.assertLess(reconstructed_series.max(), self.data_scaled[series_name].max() + original_std * 3)
    ...
    def test_scaling_consistency_across_modes(self):
        ...
        if detector_scaled.reconstructed is not None and detector_unscaled.reconstructed is not None:
    ...
    def test_metadata_scale_attribute(self):
        ...
        self.assertAlmostEqual(stored_scale, actual_std, delta=actual_std * 0.01,
        ...
        if detected['anomalies']:
            for anomaly in detected['anomalies']:
                if isinstance(anomaly, dict):
                    ...
                    self.assertLess(abs(magnitude), original_std * 100,
    ...
    def test_component_reconstruction_scale(self):
        ...
        for component_name in ['trend', 'level_shift', 'seasonality', 'holidays', 'noise', 'anomalies']:
            if component_name in detector.reconstructed_components:
                ...
                if series_name in component_df.columns:
                    ...
                    self.assertTrue(np.all(np.isfinite(component_series.dropna())),
                    ...
                    if component_name in ['trend', 'level_shift', 'seasonality', 'holidays']:
                        # These should have meaningful magnitudes
                        if component_std > 0.01:  # Only check if component is non-trivial
                            self.assertGreater(component_std, original_std * 0.001,
                                             f"{component_name} component appears to be in wrong scale")
        
        # Sum of all components should equal original data (approximately)
        if all(comp in detector.reconstructed_components for comp in ['trend', 'level_shift', 'seasonality', 'holidays', 'noise', 'anomalies']):
            reconstructed_sum = sum(
                detector.reconstructed_components[comp][series_name]
                for comp in ['trend', 'level_shift', 'seasonality', 'holidays', 'noise', 'anomalies']
            )
            
            # Mean should match
            self.assertAlmostEqual(reconstructed_sum.mean(), original_mean, delta=original_std * 0.5,
    ...
    def test_seasonality_component_scale(self):
        ...
        if isinstance(seasonality_component, dict):
            values = [v for v in seasonality_component.values() if isinstance(v, (int, float)) and np.isfinite(v)]
            if values:
                ...
                self.assertGreater(seasonal_std, original_std * 0.01,
    ...
    def test_noise_level_in_original_scale(self):
        ...
        self.assertLess(noise_level, 10,
                       "Noise level ratio seems unreasonably large")
        
        # For our synthetic data with noise_level=0.5, should be reasonable
        self.assertLess(noise_level, 2.0,
                       "Noise level too high for configured parameters")
    ...
    def test_multiple_series_scaling(self):
        ...
        for series_name in data_multi_scale.columns:
            ...
            self.assertIn('series_scale', detected)
            self.assertAlmostEqual(detected['series_scale'], original_std, delta=original_std * 0.01)
